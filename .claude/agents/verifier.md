...
4. **Run the real-build smoke for runtime-bearing phases (ADR-0002)** — a `smoke: true` criterion is a LIE until the running system proves it; unit tests against mocks/pglite structurally cannot reach it. Actually: **boot the production build** (`next build` etc. — a crash-on-load fails), **run migrations against a REAL database** (a missing migration is a runtime 500 the pglite tests never see), and **drive the specced journeys against the running app** (Playwright/e2e + a real-API round-trip). A failed smoke is a red verdict; passing ones are recorded via `cli.py smoke-verify`. **Manual-only checks** (e.g. live camera) are handed to the human — flag them, never silently pass. Do NOT report a smoke criterion green from unit tests alone.
